...
(GameID INT NOT NULL PRIMARY KEY,
 ...
 G_3PTPct NUMERIC(4,1) NULL,
 G_FTPct NUMERIC(4,1) NULL);
...
(PlayerAverageID INT NOT NULL PRIMARY KEY,
 ...
 P_3PTPct NUMERIC(4,1) NULL,
 P_FTPct NUMERIC(4,1) NULL);
...
FROM Player_Averages pa JOIN Players p ON pa.PlayerID = p.PlayerID
WHERE P_Points > 10
...
AS
SELECT GameID, DatePlayed, G_Points, G_Rebounds, G_Assists, G_Turnovers, G_Blocks, G_Steals
FROM Games

--Creating a view to present the table Players
CREATE VIEW PlayerInfo
AS
SELECT PlayerID, TeamID, P_FirstName, P_LastName, Age
FROM Players

--Creating a view to present the table Season
CREATE VIEW SeasonInfo
AS
SELECT SeasonID, SeasonYear
FROM Season

--Creating a view to present the table Teams
CREATE VIEW TeamInfo
AS
SELECT TeamID, TeamName
FROM Teams

--Creating a view to present the tables Teams_In_Games, Teams and Games
CREATE VIEW TeamsInGamesInfo
AS
SELECT DatePlayed, TeamGameID, t.TeamID, g.GameID, TeamName
FROM Teams_In_Games tig JOIN Teams t ON tig.TeamID = t.TeamID
JOIN Games g ON g.GameID = tig.GameID
...
USE WNBAStatsDB;
SELECT TeamName, DatePlayed, g.GameID, (g.G_Steals/ G_Turnovers) AS StealsToTurnovers
FROM Teams t JOIN Teams_In_Games tim ON t.TeamID = tim.TeamID
JOIN Games g ON tim.GameID = G.GameID
...
JOIN Player_Averages pa ON p.PlayerID = pa.PlayerID
...
, P_3PTPct
, P_FTPct
FROM Teams t JOIN Players p ON t.TeamID = p.TeamID
JOIN Player_Averages pa ON pa.PlayerID = p.PlayerID
WHERE Age > 30
ORDER BY Age DESC;
